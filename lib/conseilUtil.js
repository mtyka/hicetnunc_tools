...
const getCollectionForAddress = async (address) => {
    ...
    let collection = collectionResult.map((i) => {
        return { objectId: i.key.toString().replace(/.* ([0-9]{1,}$)/, '$1'), amount: Number(i.value) }
    })

    const queryChunks = chunkArray(collection.map(i => i.objectId), 20) // NOTE: consider increasing this number somewhat
    ...
    collection = collection.map(i => { return {
        ipfsHash: objectIpfsMap[i.objectId.toString()],
        ...i
    }})

    return collection.sort((a, b) => parseInt(b.objectId) - parseInt(a.objectId)) // sort descending by id – most-recently minted art first
}
...
const getMetadataIpfsMap = async (objkt_id_list) => {
    const collection = objkt_id_list;
    const queryChunks = chunkArray(collection, 400) // NOTE: consider increasing this number somewhat
    const makeObjectQuery = (keys) => {
        let mintedObjectsQuery = conseiljs.ConseilQueryBuilder.blankQuery();
        mintedObjectsQuery = conseiljs.ConseilQueryBuilder.addFields(mintedObjectsQuery, 'key_hash', 'value');
        mintedObjectsQuery = conseiljs.ConseilQueryBuilder.addPredicate(mintedObjectsQuery, 'big_map_id', conseiljs.ConseilOperator.EQ, [mainnet.nftMetadataMap])
...
const getObjects = async () => {
    ...
    let results = {};
    for(i in objectResult) {
      const objectId = objectResult[i]['value'].toString().replace(/.*Pair *([0-9]{1,}).*$/, '$1')
      const objectUrl = objectResult[i]['value'].toString().replace(/.* 0x([0-9a-z]{1,}) \}$/, '$1')
      const ipfsHash = Buffer.from(objectUrl, 'hex').toString().slice(7);
      results[objectId] = ipfsHash;
    }
    return results;
}
...
const getSwapsById = async (objectId) => {
    let swapsQuery = conseiljs.ConseilQueryBuilder.blankQuery();
    swapsQuery = conseiljs.ConseilQueryBuilder.addFields(swapsQuery, 'key', 'value');
    swapsQuery = conseiljs.ConseilQueryBuilder.addPredicate(swapsQuery, 'big_map_id', conseiljs.ConseilOperator.EQ, [mainnet.nftSwapMap])
...
const listSwaps = async () => {
    ...
    swapsQuery = conseiljs.ConseilQueryBuilder.setLimit(swapsQuery, 100_000)
...
const getObjktInfoUrl = async (objktList) => {
    const queryChunks = chunkArray(objktList, 1000)
