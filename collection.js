...
if(!tz) {
  console.log("node collection.js <wallet>");
  return 1;
}
...
audit_list = async(tokens) => {
  ...
  details.forEach((o) => {
    ...
    let line = `${baseURL}${o.token_id} by ${o.creator} Owned: ${reserve} ForSale: ${available}/${o.total}:`
...
(async () => {
  ...
  const id_set = new Set(creations.map((c) => c.objectId));
  const collection_pref = await getCollectionForAddress(tz);
  const collection = collection_pref.filter((c) => !id_set.has(c.objectId));
  await audit_list(collection);
})();
